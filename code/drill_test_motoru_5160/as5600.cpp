...
bool AS5600L::read16(uint8_t reg, uint16_t &value) {
  ...
  value &= 0x0FFF; // AS5600L má 12 bitů
  return true;
}
...
bool AS5600L::update() {
  ...
  if (delta > WRAP_THRESHOLD) {
    // např. 10 -> 4090 = pohyb zpět přes nulu
    _turnCounter--;
  } else if (delta < -WRAP_THRESHOLD) {
    // např. 4090 -> 10 = pohyb vpřed přes nulu
    _turnCounter++;
  }
...
int32_t AS5600L::getTurns() const {
  ...
  if (counts >= 0) {
    ...
    // korektní chování i pro záporné směry
    return -(((-counts) / COUNTS_PER_REV));
  }
}
