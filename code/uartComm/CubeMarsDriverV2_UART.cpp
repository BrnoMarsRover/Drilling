...
#define RQMAKS_RPM 0x80

SerialUART& driverSerial = Serial1;
...
std::vector<uint8_t> int32ToBytes(int32_t input)
{
...
void uartInit()
{
  driverSerial.begin(921600);  
}

const uint16_t crc16(std::vector<uint8_t> payload)
{
  uint16_t cksum = 0;
...
void sendUartMsg(std::vector<uint8_t> msg)
{
  driverSerial.write(msg.data(), msg.size());
/*
  Serial.println("start");
  for(uint8_t i = 0; i < msg.size(); i++)
  {
    //driverSerial.write(msg[i]);
    Serial.println(msg[i], HEX);
  }
  Serial.println("end\n");*/
}

std::vector<uint8_t> generateUartMsg(std::vector<uint8_t> payload)
{
...
void setERPM(const int32_t erpm)
{
...
void requestTmpCurrRPM()
{
  ...
  int32_t dataToRequest = RQMASK_MOSTMP | RQMASK_MOTORTMP | RQMASK_OUTCURRENT | RQMAKS_RPM;
...
std::vector<uint8_t> readPayload()
{
  ...
  enum readEnum state = start;

  while(driverSerial.available() && state != readEnum::done)
  {
    switch(state)
    {
      case readEnum::start:
      {
        if(driverSerial.read() == 2)
        {
          state = msg;
        }
        break;
      ...
      case readEnum::msg:
      {
        uint8_t length = driverSerial.read();
        output.resize(length);

        for(uint8_t i = 0; i < length; i++)
        {
          output[i] = driverSerial.read();
        }
...
*/
  if(payload.size() == 17 && payload.data()[0] == 50)
  {
    ...
    output.MOSTmp = bytesToInt16(payload.data() + begin);
    output.motorTmp = bytesToInt16(payload.data() + begin + 2);
    output.current = bytesToInt32(payload.data() + begin + 4);
    output.speed = bytesToInt32(payload.data() + begin + 8);
  }
  else
  {
    output.valid = 0;
