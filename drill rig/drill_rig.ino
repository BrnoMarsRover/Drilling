...
void initializeDrivers() {
    ...
    driver1.begin();
    driver1.toff(toffValue);
    driver1.rms_current(rmsCurrent);
    driver1.microsteps(uSteps);
    driver1.intpol(intpolSetting);
    driver1.en_spreadCycle(spreadCycleSetting);

    driver2.begin();
    driver2.toff(toffValue);
    driver2.rms_current(rmsCurrent);
    driver2.microsteps(uSteps);
    driver2.intpol(intpolSetting);
    driver2.en_spreadCycle(spreadCycleSetting);
}
...
void receiveEvent(int howMany) {
  if (Wire.available() >= 2) {
    ...
    if (receivedCommand >= 0 && receivedCommand <= 4) {
      currentState = static_cast<states>(receivedCommand);
      ...
      // Resetting error after successful state change
      if(errorStatus = 1)
        errorStatus = 0; 
